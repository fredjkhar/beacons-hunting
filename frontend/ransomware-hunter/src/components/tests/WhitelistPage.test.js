...
describe('Whitelist.vue', () => {
  ...
  test('renders component correctly', () => {
    ...
    expect(wrapper.findAll('button').length).toBe(2); // Three buttons in the component
  });

  test('adds item to destinations and updates localStorage', async () => {
    const item = 'Destination 1';
    const input = wrapper.find('input');
    const addButton = wrapper.findAll('button').at(0); // "Add to Destinations" button
  ...
  test('adds item to senders and updates localStorage', async () => {
    const item = 'Sender 1';
    const input = wrapper.find('input');
    const addButton = wrapper.findAll('button').at(1); // "Add to Senders" button
  ...
  test('removes item from destinations and updates localStorage', async () => {
    ...
    const removeButtons = wrapper.findAll('.whitelist-group-1 .remove-btn');
  ...
  test('removes item from senders and updates localStorage', async () => {
    ...
    const removeButton = wrapper.find('.whitelist-group-2 .remove-btn');
